import paputGreen from './paputGreen.svg';
import deliveryGreen from './deliveryGreen.svg';
import hamburger from './hamburger.svg';
import img1 from './img1.svg';
import map from './map.svg';
import bikeNav from './bikeNav.svg';
import wpNav from './wpNav.svg';
import instaNav from './instaNav.svg';
import musNav from './musNav.svg';
import close from './close.svg';
import cursorDefault from './cursorDefault.svg';
import cursorHover from './cursorHover.svg';
import vehicle from './vehicle.png';
import instaFooter from './instaFooter.svg';
import wpFooter from './wpFooter.svg';
import vehicleFooter from './vehicleFooter.svg';
import musFooter from './musFooter.svg';
import paputYellow from './paputYellow.svg';
import instaNavR from './instaNavR.svg';
import wpNavR from './wpNavR.svg';
import bikeNavR from './bikeNavR.svg';
import musNavR from './musNavR.svg';
import musFooterY from './musFooterY.svg';
import bikeFooterY from './bikeFooterY.svg';
import wpFooterY from './wpFooterY.svg';
import instaFooterY from './instaFooterY.svg';
import aboutImg from './aboutImg.svg';
import image1 from './image1.jpg';
import image2 from './image2.jpg';
import image3 from './image3.jpg';
import image4 from './image4.jpg';
import image5 from './image5.jpg';
import image6 from './image6.jpg';
import image7 from './image7.jpg';
import image8 from './image8.png';
import image9 from './image9.jpg';
import image10 from './image10.jpg';
import image11 from './image11.jpg';
import image12 from './image12.png';
import image13 from './image13.png';
import image14 from './image14.jpg';
import image15 from './image15.jpg';
import image16 from './image16.jpg';
import image17 from './image17.jpg';
import eventos from './eventos.svg';
import event1 from './event1.jpg';
import shop from './shop.svg';
import prod1 from './prod1.png';
import prod2 from './prod2.png';
import prod3 from './prod3.png';
import prod4 from './prod4.jpg';
import prod5 from './prod5.jpg';
import prod6 from './prod6.jpg';
import prod7 from './prod7.jpg';
import burger1 from './burger1.png';
import burger2 from './burger2.png';
import burger3 from './burger3.png';
import burger4 from './burger4.png';
import burger5 from './burger5.png';
import burger6 from './burger6.png';
import burger7 from './burger7.png';
import burger8 from './burger8.png';
import burger9 from './burger9.png';
import burger10 from './burger10.png';
import grill from './grill.jpg';
import icon1 from './icon1.svg';
import icon2 from './icon2.svg';
import heroBg from './heroBg.jpg';
import tag from './tag.svg';
import tagGreen from './tagGreen.svg';
import slidingImg from './slidingImg.jpg';
import nastras from './nastras.svg';

export default {
  paputGreen,
  deliveryGreen,
  hamburger,
  img1,
  map,
  bikeNav,
  wpNav,
  instaNav,
  musNav,
  close,
  cursorDefault,
  cursorHover,
  vehicle,
  instaFooter,
  wpFooter,
  vehicleFooter,
  musFooter,
  paputYellow,
  instaNavR,
  wpNavR,
  bikeNavR,
  musNavR,
  musFooterY,
  bikeFooterY,
  wpFooterY,
  instaFooterY,
  aboutImg,
  image1,
  image2,
  image3,
  image4,
  image5,
  image6,
  image7,
  image8,
  image9,
  image10,
  image11,
  image12,
  image13,
  image14,
  image15,
  image16,
  image17,
  eventos,
  event1,
  prod1,
  prod2,
  prod3,
  prod4,
  prod5,
  prod6,
  prod7,
  shop,
  burger1,
  burger2,
  burger3,
  burger4,
  burger5,
  burger6,
  burger7,
  burger8,
  burger9,
  burger10,
  grill,
  icon1,
  icon2,
  heroBg,
  tag,
  tagGreen,
  slidingImg,
  nastras,
};
